...
normalization rules — since 0.7.4 both live in `task_core/db/identifiers.py`,
one module below the publisher, which does not change the argument: they are
still PostgreSQL constraints rather than scaffold conventions.
`source_state.py` shares the pattern rather than keeping a second copy.
