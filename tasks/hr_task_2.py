...
def is_blank_header_label(value):
    # Shared by normalize_header_label() and check_no_duplicate_headers()
    # below, deliberately -- both need the exact same definition of
    # "blank", and having two, separately-maintained versions is exactly
    # how they drifted apart before: normalize_header_label() already
    # correctly recognized a genuinely-empty cell (None) or an actual NaN
    # float, but nothing here originally also treated an empty string as
    # blank, and check_no_duplicate_headers() didn't share this logic at
    # all, checking labels as they came back out of a pandas DataFrame's
    # .columns instead -- which matters because pandas's own
    # `df.columns = [...]` assignment silently converts a None value to
    # np.nan (a float) during Index construction, confirmed directly, not
    # assumed. A caller reading normalize_header_label(cell) directly
    # sees None; a caller reading it back via df.columns after an
    # assignment sees nan instead, invisibly -- both need recognizing as
    # the same "blank" here.
    if value is None:
        return True
...
def check_no_duplicate_headers(labels, file_name):
    ...
    # actual cause.
    #
    # Blank labels (is_blank_header_label() above -- None, '', or a
    # genuine NaN float) are excluded from this check entirely -- found
    # against a real production file (multiple blank header cells,
    # previously rejected as "duplicates" of each other), confirmed
    # directly: multiple blank/unnamed header cells in the same sheet is
    # completely normal (spacer columns, columns beyond the actual data's
    # width that openpyxl still reports), and every caller of this
    # function only ever selects columns by their real, specific name
    # afterward (e.g. hdr.loc[..., ['Блок', 'ФИО', ...]]), never by a
    # blank value. A blank header can never trigger the df['col'] returns
    # a DataFrame problem this check exists to prevent, since nothing
    # ever looks one up by that value -- it's dropped, implicitly and
    # harmlessly, by every caller's own explicit column selection.
    seen = {}
    duplicates = {}
