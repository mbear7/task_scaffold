...
- **A quoting mode that converts values could leak a raw `ValueError`, with
  ...
  `abc` escaped the `CsvReadError` contract entirely. Wrapped as
  `CsvReadError` without interpolating the cause; the value stays reachable
  through `__cause__` for local debugging.
- **An all-unusable CSV file set now names itself.** That failure belongs to
  the set rather than to any member, so it reports the selection root and
  pattern (`source=C:eed\*.csv`), read from the selection already made —
...
- Under **`QUOTE_NONNUMERIC`** values are *not* strings — the standard
  library converts them, and task_core neither adds inference nor
  suppresses the library's.
